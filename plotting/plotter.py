...
class Plotter:
    ...
    def __init__(self, path: str):
        ...
        self.total_energy, self.coarse_energy = self.reader.get_coarse_energy()

        self.fine_energy = self.reader.get_fine_energy()


    def bar_plot(self, ax, data, layer_numbers, 
        colors=None, total_width=0.8, single_width=1, legend=True, ylabel='Energy', title=''):
    ...
    def stacked_bar_plot(self, ax, data, layer_numbers, stack_labels, 
        colors=None, total_width=0.8, single_width=1, legend=True, ylabel='Energy', title=''):
        ...
        data: dictionary
            A dictionary containing the data we want to plot. Keys are the names of the
            data, the items is a nested list of the values. One list for each stack
        ...
        for i, (name, nested_list) in enumerate(data.items()):
            ...
            for j, values in enumerate(nested_list):
                ...
                bar = ax.bar(x, values, width=bar_width * single_width, 
                        bottom=stack_below, color=colors[j % len(colors)])

                # Add 'min_en' or 'max_ut' to bottom of correct bar
                if j == 0:
                    for x_pos in x:
                        ax.text(x_pos, 150, name.replace('_',' '), ha='center', fontsize=10, color='k')
    ...
    def plot_energy(self, total_width=.6, single_width=.9, colors=None):


        fig1, ax1 = plt.subplots()
        self.bar_plot(ax1, self.total_energy, self.layer_numbers, 
                total_width=total_width, single_width=single_width, colors=colors, 
                title='Total energy')

        coarse_stack_labels = ['Memory access energy', 'MAC energy']
        fig2, ax2 = plt.subplots()
        self.stacked_bar_plot(ax2, self.coarse_energy, self.layer_numbers, coarse_stack_labels, 
                total_width=total_width, single_width=single_width, colors=colors, 
                title='Coarse grained energy')

        # Construct the legend labels for the stacks in the bar plot
        fine_stack_labels = []
        mem_hierarchy_labels = self.reader.mem_hierarchy_labels
        ...
        self.stacked_bar_plot(ax3, self.fine_energy, self.layer_numbers, fine_stack_labels, 
                total_width=total_width, single_width=single_width, colors=colors, 
